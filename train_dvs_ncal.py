...
from models.snndarts_retrain.LEAStereo import LEAStereo, LEAStereo2, LEAStereo3
import fitlog
import torch.nn.functional as F

parser = argparse.ArgumentParser("cifar")
parser.add_argument('--data', type=str, default='../data/ncal', help='location of the data corpus')
...
def main():
    ...
    leastereo = LEAStereo3(2, CIFAR_CLASSES, args=args)
    model = leastereo
    model = model.cuda()
    ...
    train_data = NCaltech101(root=args.data,  data_type='frame', frames_number=T, split_by='number')
    test_set = NCaltech101(root=args.data,  data_type='frame', frames_number=T, split_by='number')
    train_data, valid_data = random_split(train_data, [0.2, 0.8])
...
def train(train_queue, model, criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        optimizer.zero_grad()
        logits, logits_aux = model(input, T, param)

...
import spikingjelly.datasets.cifar10_dvs
import torch
import utils
import logging
import argparse
...
from MyOptimizer.sgd import SGD
from MyOptimizer.prosgd import ProSGD
import torchvision
import math
import random
...
parser.add_argument('--learning_rate', type=float, default=0.01, help='init le1arning rate')
...
args.save = 'sgd-{}-search-{}-{}'.format(args.seed,args.save, time.strftime("%Y%m%d-%H%M%S"))
utils.create_exp_dir(args.save, scripts_to_save=glob.glob('*.py'))

log_format = '%(asctime)s %(message)s'
logging.basicConfig(stream=sys.stdout, level=logging.INFO,
                    format=log_format, datefmt='%m/%d %I:%M:%S %p')
fh = logging.FileHandler(os.path.join(args.save, 'lognew.txt'))
...
import spikingjelly
import tonic
from tonic import transforms as snn_transforms
...
def main():
    ...
    # train_data = CIFAR10DVS(root=args.data, data_type='event', frames_number=T, 
    #                         split_by='number', transform=torchvision.transforms.ToTensor())
    # test_set = CIFAR10DVS(root=args.data, train=False, data_type='event', frames_number=T, split_by='number')
    # if args.CIFAR10:
    ...
    filter_time=10000
    frame_transform = snn_transforms.Compose([
                                        snn_transforms.Denoise(filter_time=filter_time), 
                                        snn_transforms.ToFrame(sensor_size=sensor_size, n_time_bins=T), # frame 个数
                                        torch.from_numpy,
                                        transforms.RandomRotation([-10,10])
                                    ])
    dataset = tonic.datasets.CIFAR10DVS(save_to=args.data, transform=frame_transform)
    train_data, valid_data, test_data = random_split(dataset, [0.4, 0.4, 0.2])
    ...
    train_loader = DataLoader(train_data, batch_size=bs, collate_fn=tonic.collation.PadTensors(batch_first=False), 
                            shuffle=True, pin_memory=True, num_workers=16)
    valid_loader = DataLoader(valid_dataset, batch_size=val_batch, collate_fn=tonic.collation.PadTensors(batch_first=False),  
                            shuffle=True, pin_memory=True, num_workers=16)
    test_loader = DataLoader(test_dataset, batch_size=bs, collate_fn=tonic.collation.PadTensors(batch_first=False),  
    ...
    logits_tr = model(input_tr, timestamp=T)
    ...
    architect_optimizer = ProSGD(arch_param,
                                lr=args.arch_learning_rate, 
                                momentum=0.9)
    # architect_optimizer = torch.optim.Adam(arch_param,
    ...
    with torch.no_grad():
        ...
        logits_search = model(input_search, timestamp=T)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (input_tr, target_tr) in enumerate(train_queue):
            ...
            nn.utils.clip_grad_norm_(model.feature.parameters(), args.grad_clip)
            optimizer.step()
            architect_optimizer.step()
            # arch_param[0].data.copy_(projectionA(arch_param[0]) )          
            # arch_param[1].data.copy_(projectionA(arch_param[1]))
            optimizer.zero_grad()
            ...
            if step % args.report_freq == 0:
                ...
                logging.info('train %03d %e %f %f val %e %f %f norm %f %f %f %f ', 
                             step, objs.avg, top1.avg, top5.avg, objs_val.avg, top1_val.avg, top5_val.avg,
...
def infer(valid_queue, model, criterion):
    ...
    with torch.no_grad():
        for step, (input, target) in enumerate(valid_queue):
            ...
            logits = model(input, timestamp=T)

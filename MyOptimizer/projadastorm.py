...
__all__ = ['ProAdaStorm', 'proadamstorm']


class ProAdaStorm(Optimizer):
...
ProAdaStorm.__doc__ = r"""Implements Adam algorithm.
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    for ((device_params, device_grads, device_grads_old, device_exp_avgs, device_exp_avg_sqs,
        ...
        # momentum = beta1
        torch._foreach_add_(device_exp_avgs, device_grads_old, alpha=-1*pi)
        ...
        step_size = _stack_if_compiling([lr * (-1) for bc in bias_correction1])
        ...
        params_[0].data = projectionA(params_[0])           
        params_[1].data = projectionA(params_[1])

        torch._foreach_mul_(params_, etak)
        torch._foreach_add_(params_, device_params_old, alpha=1-etak)

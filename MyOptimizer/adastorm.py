...
class AdaStorm(Optimizer):
    ...
    def step(self, etak, d_p_list_old=None, closure=None):
...
AdaStorm.__doc__ = r"""Implements Adam algorithm.
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    grouped_tensors = _group_tensors_by_device_and_dtype([params, grads,grads_old, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps])
    for ((device_params, device_grads, device_grads_old, device_exp_avgs, device_exp_avg_sqs, device_max_exp_avg_sqs, device_state_steps),_) in grouped_tensors.values():
        ...
        device_grads_old = [torch.view_as_real(x) if torch.is_complex(x) else x for x in device_grads_old]
        ...
        # Decay the first and second moment running average coefficient
        momentum = np.clip(beta1 * (etak ** 2), a_min=0.1, a_max=1)
        # momentum = beta1 * (etak ** 2)
        # momentum = beta1
        # torch._foreach_mul_(device_exp_avgs, 1-momentum)
        # torch._foreach_add_(device_exp_avgs, device_grads, alpha= momentum)
        # torch._foreach_add_(device_exp_avgs, device_grads, alpha=1-momentum)
        # torch._foreach_add_(device_exp_avgs, device_grads_old, alpha=-1+momentum)

        torch._foreach_add_(device_exp_avgs, device_grads_old, alpha=-1)
        torch._foreach_mul_(device_exp_avgs, 1-momentum)
        torch._foreach_add_(device_exp_avgs, device_grads, alpha=1)
        
        torch._foreach_mul_(device_exp_avg_sqs, beta2)
        torch._foreach_addcmul_(device_exp_avg_sqs, device_grads, device_grads, 1 - beta2)

        bias_correction1 = [1 - momentum ** _get_value(step) for step in device_state_steps]
        bias_correction2 = [1 - beta2 ** _get_value(step) for step in device_state_steps]

        step_size = _stack_if_compiling([lr * (-etak) for bc in bias_correction1])
        ...
        # denom = []
        # for g in device_exp_avgs:
        #     denom.append(g/(torch.sqrt(torch.norm(g))+1e-6))

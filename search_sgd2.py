...
# tonic_logger = logging.getLogger('tonic')
# tonic_logger.setLevel(logging.INFO)
# tonic_logger.addHandler(fh)
# fitlog_debug = False
# fitlog_debug = True
# if fitlog_debug:
#     fitlog.debug()
...
def main():
    ...
    # class ToTensorAndCorrect:
    #     def __call__(self, sample):
    #         if isinstance(sample, np.lib.npyio.NpzFile):
    #             # 从 .npz 文件中提取数组
    #             sample = sample['arr_0']  # 根据你的 .npz 文件结构调整这里
    #         return torch.tensor(sample, dtype=torch.float32)
    # transform = transforms.Compose([
    #     ToTensorAndCorrect()  # 使用自定义转换函数
    # ])
    # dataset = CIFAR10DVS(root=args.data, data_type='event', frames_number=T, 
    #                         split_by='number', transform=transform)
    ...
    #                                     snn_transforms.ToFrame(sensor_size=(2,128,128), n_time_bins=T), # frame 个数
    #                                     torch.from_numpy,
    ...
    valid_queue = DataLoader(valid_data, batch_size=val_batch,  
                            shuffle=True, pin_memory=True, num_workers=16)
    # test_loader = DataLoader(test_data, batch_size=args.batch_size, collate_fn=tonic.collation.PadTensors(batch_first=False),  

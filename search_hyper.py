...
args.save = 'vr2-{}-search-{}-{}'.format(args.seed,args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main(arch, lr):
    ...
    if args.CIFAR10:
        train_queue = torch.utils.data.DataLoader(
            ...
            pin_memory=True, num_workers=16,worker_init_fn=seed_worker)   # 25000
...
if __name__ == '__main__':
    arch_sequence = np.logspace(0, -2, num=3, base=10)
    lr_sequence = np.logspace(0, -3, num=4, base=10)
    bestValue = 0
    bestArch = 0
    bestLr = 0
    for arch in arch_sequence:
        for lr in lr_sequence: 
            val_acc=main(arch,lr)
            if val_acc >= bestValue:
                bestLr = lr
                bestArch = arch
                bestValue = val_acc
            logging.info(' best valid %f %f %f', val_acc, bestLr, bestArch)

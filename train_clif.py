...
from models.snndarts_retrain.LEAStereo import LEAStereo, LEAStereo_ImageNet,LEAStereo_clif
import fitlog
...
args.save = 'clif_eval_{}_time_{}_{}-{}-{}'.format(args.dataset, args.time_stamp, args.author, args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
    ...
    leastereo = LEAStereo_clif(init_channels=3, CIFAR_CLASSES=CIFAR_CLASSES, args=args)
    model = leastereo
    model = model.cuda()
    ...
    for epoch in range(args.epochs):
        ...
        train_acc, train_obj = train(train_queue, model, criterion, optimizer, time_stamp)
        logging.info('train_acc %f', train_acc)
        fitlog.add_metric(train_acc,epoch,'train_top1')
        fitlog.add_metric(train_obj,epoch,'train_loss')

        valid_acc, valid_obj = infer(valid_queue, model, criterion, time_stamp)
...
def train(train_queue, model, criterion, optimizer, time_stamp):
    param = {'mode':'optimal'}
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        logits, logits_aux, _, _, _, _ = model(input, time_stamp,  param)
...
def infer(valid_queue, model, criterion, time_stamp):
    param = {'mode':'optimal'}
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        logits, logits_aux, A_spikes, T_spikes, _, _ = model(input, time_stamp,  param)
        loss = criterion(logits, target)
        A_s += A_spikes
        T_s += T_spikes
        prec1, prec5 = utils.accuracy(logits, target, topk=(1, 5))
    ...
    logging.info('activate rate %f', A_s/T_s)
    return top1.avg, objs.avg

...
import numpy as np
from models.snndarts_search.build_model_2d import AutoFeature, AutoFeature2
...
class AutoStereo(nn.Module):
    def __init__(self, init_channels=3, CIFAR_CLASSES=10, args=None):
        ...
        self.feature  = AutoFeature(init_channels, CIFAR_CLASSES=CIFAR_CLASSES, args=args)
    ...
    def forward(self, input, timestamp=8): 
...
class AutoStereo2(nn.Module):
    def __init__(self, init_channels=3, CIFAR_CLASSES=10, args=None):
        super(AutoStereo2, self).__init__()
        p=0.0
        network_path_fea = [0,1,2,3]
        network_path_fea = np.array(network_path_fea)
        network_arch_fea = network_layer_to_space(network_path_fea)
        ...
        self.feature  = AutoFeature2(init_channels, CIFAR_CLASSES=CIFAR_CLASSES, args=args)
    ...
    def forward(self, input, timestamp=6): 
        ...
        logits = torch.zeros((timestamp,input.shape[1],self.class_num)).cuda()
        for i in range(timestamp):
            param['mixed_at_mem'] = False
            if i == 0:
                param['is_first'] = True
            ...
            feature_out = self.feature(input[i], param) 
            # pooling_out = self.global_pooling(feature_out)
            # logits_buf = self.classifier(pooling_out.view(pooling_out.size(0),-1))

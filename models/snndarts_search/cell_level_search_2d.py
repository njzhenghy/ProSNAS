...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights, n_gammas, param):
        ...
        if self.partial_channel == 1:
            ...
            temp1 = sum(w * op(xtemp, n_gammas, param) for w, op in zip(weights, self._ops))
            ...
            for i in range(len(self._ops)):
                ...
                if isinstance(self._ops[i], nn.Sequential) or isinstance(self._ops[i], Identity):
                    opt_out = self._ops[i](x, n_gammas, param)
                else:
...
class Cell(nn.Module):
    ...
    def forward(self, s0, s1_down, s1_same, s1_up, n_alphas, n_gammas, param):
            ...
            for i in range(self._steps):
                ...
                for j, h in enumerate(states):
                    ...
                    if self._ops_down[branch_index] is None:
                        continue           
                    new_state = self._ops_down[branch_index](h, n_alphas[branch_index], n_gammas[branch_index], param)
                    if param['is_first']:
            ...
            for i in range(self._steps):
                ...
                for j, h in enumerate(states):
                    ...
                    if self._ops_same[branch_index] is None:
                        continue            
                    new_state = self._ops_same[branch_index](h, n_alphas[branch_index], n_gammas[branch_index], param)
                    if param['is_first']:
            ...
            for i in range(self._steps):
                ...
                for j, h in enumerate(states):
                    ...
                    if self._ops_up[branch_index] is None:
                        continue               
                    new_state = self._ops_up[branch_index](h, n_alphas[branch_index], n_gammas[branch_index], param)
                    if param['is_first']:

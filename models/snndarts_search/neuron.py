...
from models.snndarts_search.surrogates import Rectangle
...
class ComplementaryLIFNeuron(nn.Module):
    def __init__(self, thresh=1.0, tau=0.5, gama=1.0):
        super(ComplementaryLIFNeuron, self).__init__()
        self.act = ZIF.apply  # 使用 ZIF 作为 surrogate function
        self.thresh = thresh  # 阈值
        self.tau = tau  # 时间常数
        self.gama = gama  # ZIF 近似参数
    ...
    def forward(self, x):
        ...
        - `x`: 输入电流，形状 `[batch, *]`
        """
        self.mem = self.tau * self.mem + x  # 充电
        spike = self.act(self.mem - self.thresh, self.gama)  # 触发
        self.m = self.m * torch.sigmoid((1.0 - self.tau) * self.mem) + spike  # 互补变量更新
        self.mem = self.mem - spike * (self.thresh + torch.sigmoid(self.m))  # 复位
...
class CLIFSpike(nn.Module):
    def __init__(self, tau: float=2.):
        ...
        self.spike_func = Rectangle()  # 使用 Rectangle 作为脉冲激活函数

        self.v_th = 1.0  # 阈值
        self.gamma = 1 - 1.0 / tau  # 记忆因子
    ...
    def forward(self, x):
        # ✅ 单步计算
        self.u = self.gamma * self.u + x
        spike = self.spike_func(self.u - self.v_th)
        self.m = self.m * torch.sigmoid((1.0 - self.gamma) * self.u) + spike
        self.u = self.u - spike * (self.v_th + torch.sigmoid(self.m))
...
class ComplementaryLIFNeuron2(LIFNode_sj):
